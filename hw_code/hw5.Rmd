...
which shows that the communalities in a factor analysis model are unaffected by the transformation Λ∗ = ΛM.
...
The results of the maximum likelihood test indicate that the p-values for three factors is not significant and thus may offer an adequate solution. 

```{r}
factanal(covmat = item_cor, factors = 1, method = "mle") 
```

```{r}
library(psych)
fa_1 <- fa(item_cor, factors = 1)
fa_1
```
